...
-- register & deregister
function CHARACTER_CHANGE_REGISTER_ADD_ITEM(add_index, reg_char_index)
	local frame = ui.GetFrame("character_change_register");
	...
	for i = 0, count - 1 do
		...
		if child ~= nil and string.find(child:GetName(), "register_slot_") ~= nil then
			...
			if slot_index == add_index then
				...
				local yes_scp = string.format("DO_CHARACTER_CHANGE_REGISTER(%d, %d)", reg_char_index, slot_index);
...
function DO_CHARACTER_CHANGE_REGISTER(reg_char_index, slot_index)
	if character_change.IsRegistedCharacterByIndex(reg_char_index) == false then
		character_change.RegisterMyCharacter(reg_char_index, slot_index);
...
function CHARACTER_CHANGE_REGISTER_REMOVE_ITEM(parent, btn)
	...
	if ctrl_set ~= nil then
		...
		local selected_index = ctrl_set:GetUserIValue("selected_index");
		local yes_scp = string.format("DO_CHARACTER_CHANGE_DEREGISTER(%d)", selected_index);
		ui.MsgBox(str, yes_scp, "None");
	end
end

function DO_CHARACTER_CHANGE_DEREGISTER(selected_index)
	if character_change.IsRegistedCharacterByIndex(selected_index) == true then
		character_change.DeregisterByCharacter(selected_index);
	end
end
